...
<div class="grid">
  <div class="card">
    <details open>
      ...
      <form method="post">
        ...
        <?php if($edit_suite): ?><input type="hidden" name="action" value="update_suite"><input type="hidden" name="suite_id" value="<?=$edit_suite['suite_id']?>"><?php else: ?><input type="hidden" name="action" value="add_suite"><?php endif; ?>
        <label>Name</label>
  ...
  <div class="card">
    <details open>
      ...
      <form method="post">
        <input type="hidden" name="csrf" value="<?=$CSRF?>">
        <?php if($edit_bench): ?><input type="hidden" name="action" value="update_benchmark"><input type="hidden" name="benchmark_id" value="<?=$edit_bench['benchmark_id']?>"><?php else: ?><input type="hidden" name="action" value="add_benchmark"><?php endif; ?>
        <label>Suite</label>
  ...
  <div class="card">
    <details open>
      ...
      <form method="post">
        <input type="hidden" name="csrf" value="<?=$CSRF?>">
        <?php if($edit_tool): ?><input type="hidden" name="action" value="update_tool"><input type="hidden" name="tool_id" value="<?=$edit_tool['tool_id']?>"><?php else: ?><input type="hidden" name="action" value="add_tool"><?php endif; ?>
        <label>Name</label>
    ...
    <details open>
      ...
      <form method="post">
        ...
        <div class="row">
          <div>
            <label>Suite</label>
            <select name="suite_id">
              <option value="">—</option>
              <?php foreach($all_suites as $s): ?>
                <option value="<?=$s['suite_id']?>"><?=h($s['name'].($s['variation']?(' — '.$s['variation']):''))?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            <label>Benchmark (ID)</label>
            <input type="number" name="benchmark_id" placeholder="e.g. 12">
            <div class="muted">Tip: see IDs in Benchmarks tab anchors.</div>
          </div>
        </div>
        ...
        <div class="row">
          ...
          <div>
            <label>Tool Release (ID)</label>
            <input type="number" name="tool_release_id" placeholder="optional">
          </div>
        </div>

        <div class="row3">
          <div><label>FOM1</label><input type="number" step="any" name="fom1"></div>
